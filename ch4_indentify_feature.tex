\chapter{Identifying Usage of Boogie Features}
...
\paragraph{Subset Checker and Source Code.} Before deciding on which feature to choose,
...
\paragraph{Exceptions} Another source is to look at
\lstinline[columns=fixed]{NotImplementedException} and
\lstinline[columns=fixed]{NotSupportedException}
in the code of the proof generation.
From this we gather, that it does not support \lstinline[columns=fixed]{arguments} type encoding,
...
One quick way to evaluate is to abuse the existing subset checker to collect usages.
This limits us to mainly analyze the ones listed in the checker.
...
Currently, the subset checker collects the first unsupported feature and reports it.
...
For that we first take all the integration tests from the official Dafny repo\footnote{\url{https://github.com/dafny-lang/dafny/tree/master/Source/IntegrationTests}} and compile them to Boogie program.
...
quite a sizeable portion \lstinline[columns=fixed]{free} and
\lstinline[columns=fixed]{where} clauses on procedures,
and few use Bitvectors.


\subsection{Map Usage in Dafny Tests}

It is surprising, that all input programs (that reach the subset checker) use maps.
The reason becomes clear, when one tries to compile a very simple Dafny program:

\begin{lstlisting}[caption={Simple Dafny Program}]
method Succeeds() {
    assert true;
}
\end{lstlisting}

Even this simple program, generates an almost 3000 line long Boogie program, including the following map type:
\lstinline[columns=fixed]{type Heap = [ref][Field]Box;}.
...
\footnote{\url{https://github.com/boogie-org/boogie/tree/master/Test}}.
...
\begin{lstlisting}[caption={Compiled Dafny to Boogie Program Excerpt}]
type Heap = [ref][Field]Box;

revealed function {:inline} read(H: Heap, r: ref, 
  f: Field) : Box
{
  H[r][f]
}

revealed function {:inline} update(H: Heap, r: ref, 
  f: Field, v: Box) : Heap
{
  H[r := H[r][f := v]]
}
...
Maps are used with abstract types, such as \lstinline[columns=fixed]{ref, Box, Field}.
...
Nesting on the range type is more popular, for example the type 
\lstinline[columns=fixed]{[[ref][Field]Box,Box,Box,Box]bool} appears more than 100 times.
There also seem to be tests with much higher arity, with one map having 300 \lstinline[columns=fixed]{Box} entries in the range type.
...
Looking at the Dafny test folder in the Boogie repository~\footnote{\url{https://github.com/boogie-org/boogie/tree/master/Test/dafny}},
it seems that polymorphic maps were used in the past but were later abandoned~\footnote{\url{https://github.com/dafny-lang/dafny/pull/4020}}~\textsuperscript{,}~\footnote{\url{https://github.com/dafny-lang/dafny/pull/4597}}
...
There is no explicit axiom in the VC enforcing it
(see \Cref{sec:vc-map-axioms}).
...
\begin{lstlisting}[caption={Extensionality Example}]
  var a: [int]int;
  var b: [int]int;
  var m: [[int]int]int;
...
Here we created two maps \lstinline[columns=fixed]{a} and \lstinline[columns=fixed]{b}, that behave the same.
If \lstinline[columns=fixed]{a} and \lstinline[columns=fixed]{b} would be considered differently in our semantics,
