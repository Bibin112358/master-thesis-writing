...
Another source is not look at \texttt{NotImplementedException} and \texttt{NotSupportedException}
in the code of the proof generation:
\begin{itemize}
  \item f
\end{itemize}
\begin{verbatim}
  generall NotImplemented or NotSupportedException things in source code (Do I even know what these do?)
  - TypeEncoding not in [Predicate, Monomorphic]
  - non DeclFreeProverContext
  - Proof generation does not support this case: incarnation on lhs of assignment
  - cachich verification results
\end{verbatim}

not explaining every feature

\subsection{Evaluating Feautures}

\begin{verbatim}

testing with dafny test cases
- gathering tests (2000 tests)
- adjust checker, write script to run all tests
  - script `BoogieDriver $b | sort | uniq'
  - `check.sh | sort | uniq -c | sort -nr`
- all use map
- run a simple test -> generates large boogie program
- maps still most used missing feature
- analyze simple case in which way
- example
- conclusion: no polymorphism, nested, abstract types, axioms quantifiying (so infinite), ...
...
\subsection{Chosen Feature}

map and its restrictions

note
- viper uses polymorphic maps
- dafny does not use it anymore

\subsection{Caveats}

\begin{verbatim}
caveat
- simple already contains our answer
- only collected features from subsetchecker
- checked everything (even the ones that do not verify)
