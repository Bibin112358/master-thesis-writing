...
We want to shed some light to possible other apporaches, we were considering.

% HOAS? (not tried really)
  
\subsubsection{Semantics based on typed used in input program}

We had the idea, of collecting all types in the input Boogie program,
and using this information to create a datatype for map type used in the input program.
For example, if we have a \texttt{[int]int} map, we would model is as an \texttt{int => int} function in Isabelle.
This stands in contrast to our current approach of modelling it as a \texttt{'a val0 => 'a val0} function,
between all primitive values and enforcing type safety with an well formedness invariant.
One issue we had with this, was that we had no type to represent abstract types.
Later, we found that we could potentially use Isabelle's \texttt{typedecl} to create a type for these abstract types,
and use these to represent the domain and codomain of our maps.

The advantage of this approach is that is necessitates much of the need for the well formedness definition.
The disadvantage is, that one would need to collect all types, and instantiate the map semantics,
with all the used map types, and define select and store with all the possible type combinations.
It is not clear, which approach would lead to a better semantics.


\subsubsection{Context/Heap model}

One early idea was to have a separate structure, that contains all maps
and use an identifier to access it: \texttt{maps :: int => val => val"}.
The map then stores the identifier \texttt{MapV int}.
This structure would then have to be passed around everywhere and updated accordingly.
We decided not to use that, as it creates a thing we have to pass around everywhere,
and that is updated during the process.
We also expect the reasoning to be harder, as we are essentially introducing a heap to store our maps.
The question is also, which identifier should be used, and if integers even are an appropriate identifier, especially if we want to quantify over all maps.
We also expect that the notion of equality and estensionality will not be easy with such a model.
This model seems to be good for implementing a runtime, but not for logically modelling.


\subsubsection{Quotient Types}

We often had issues with our map datatype being able to represent more things than their Boogie counter-part.
Either being able to define bogous values, such as maps that are not type safe,
or having multiple representation for the same Boogie map.
That is the motivation behind our well formedness definition \ref{sec:wf-definition}.
Another potential avenue to explore are quotient types.
They allow to define a new type, while having 'multiple' representation of the same value under the hood.
Due to lack of time and insufficient Isabelle knwoledge, we did not explore this,
but this might be a promising avenue to simplify map semantics in the future.
Especially, with potentially allowing an association list, to model a list of updates,
while having a function to fall back on.
And using quotienting to enable extensionality between maps, that were built differently,
but behave the same.
...
\subsubsection{Lambda Calculus Deep Embedding}

The most promising alternative approach is deeper embedding.
Our solution is a shallow embedding of Boogie maps to preexisting implementation of Isabelle functions.
Another approach would be to store not a mathematical function, but a Boogie expression with a variable binding.
Boogie maps seem to closely resembel lambda calculus.
Map select \texttt{m[k]} would correspond to function application \texttt{m k},
map store \texttt{m[k := v]} to function update \texttt{$\lambda$ x. if x = k then y else m x}
and lambda expressions to function $\lambda$-abastractions.
And with polymorphic map type \ref{sec:semantics-boogie-maps}, the type system would resembel System F. % TODO ref
That means we can potentially design the formal semantic of Boogie maps as a formalization of System F.

This would entail dealing with variable and binders.
This is also the main reason, why we first went with the simpler shallow approach of embedding directly to Isabelle functions, as having expressions as values and variable binders
sound like a lot of changes to the existing semantics.
For formalizing the more advanced features of maps, like unbounded nestings and polymorphism,
this might have to be the approach.
For our simpler use case, our semantics should be easier.

One question is why such deep embedding would not cause issues with Cantor's Theorem \ref{sec:cantor}.
The reason is, with the deep embedding we are not representing every possible mathematical function,
but only the functions expressible in Boogie.
For example $[int]bool$, could be used to represent a set of integers in Boogie.
The cardinality of all sets of integeres is uncountable.
But we all sets of integers that we can represent and produce in finite amount of time are countable.
\footnote{Or at least not larger than the types available. Using reals one can potentially express uncountable functions.}
So if we define our semantic such that $[int]bool$ only represents the representable functions,
this would not cause the same cardinality issues as with mathematical functions in Isabelle.


\subsubsection{Other Logic or Interactive Proof Assistants}

One question we asked ourselve, is Isabelle/HOL the right tool to define map semantics?
Since the extensive previous work was build on Isabelle/HOL, we decided to continue with it.
If we extends our map semantics to adjust the amount of supported nesting levels on the input program,
then our datatype becomes dependent on the number of nesting levels.
This sounds like we would want depedent types %TODO ref
which might help define this more cleanly.
Another part where dependent types might be useful for out well formed type definition.
This is basically a contrained type, we build from a set.
And this is dependent on our inductive definition.
Maybe using dependent types, the whole well formedness thing might not be necessary,
as one might restrict our function using the Boogie types, which are represented as values in  our Isabelle formalization.


\subsubsection{New Boogie Language: B3}

There seems to be work by the dafny developers for a new intermediate verification language: B3 \footnote{https://github.com/dafny-lang/b3}.
The question is, if this might superceed the current Boogie IVL,
does it make sense to shift focus on bulding foundational verification tools for that?
It seems still quite new, and judging from the commits Boogie IVL is more active,
but it seems worth to keep an eye on. 
...
\subsubsection{Improve current Limitations}
Clearly, one potential work for the future is to improve the current limitations of our implementation \ref{sec:limitations}.
Meaning, supporting multi-arity maps, the convenient map assign syntax, showing type safety lemmas for maps
and strengthening proof tactics to enable more automatic proof verifications.


\subsubsection{Map Nesting Levels}
Our solution supports map witt up to three levels of nestings in the domain types.
There is no limit for nesting range types, as we can safely recurse on them, see Section~\ref{sec:cantor}.
Nesting domains seems to be quite rare on the examples we found, so this serves a practical limit.
But of course a more general semantics is desirable, so extending our approach to support an bounded,
or even unbounded, number of nestings is nice.
One way to approach it would be to figure out the nesting level needed for the input program,
and then automatically generate the map semantics, its definitions and proofs accordingly.
Even better would be to see, if one can define the semantic, that works in any cases,
without having to generate the semantics based on the input, dropping the program-dependent semantics property.


\subsubsection{Support other Boogie Features}
One of the motivation listed in Section~\ref{sec:motivation} is to eventually support Dafny program,
or in general to enable a pipeline of foundational verifiers.
We fixed the most prominent limitation on that way, but there are still hurdles,
stopping us from verifying even the simplest Dafny program.
Currently, the proof generation fails, since it does not support procedures marked with \texttt{free} postconditions.
Ignoring that, there are still some value conversion functions not implemented yet,
for example a real to int function.
Supporting these next, would bring us closer to being able to foundationally verify Dafny programs.
