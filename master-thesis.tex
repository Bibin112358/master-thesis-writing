...
For simplicity, we do not support this feature,
...
Supporting maps in their full semantics is beyond the scope of this first step.
...
This would make the development more difficult, as generics and polymorphism are challenging topics, % TODO ref
...
In one Dafny case, both keys and values are maps.
Empirically, it appears to use only one level of nesting in keys,
so supporting two levels of map nesting in keys is sufficient for now.
...
and that parts of the semantics may depend on the input program.
...
but we expect that the final solution can be expanded
...
for future work.
This means that we should avoid hard-coding the number of nesting levels in the semantics.
As the current proofgen is built on the polymorphic type encoding of Boogie,
...
A natural question is whether extensionality is really necessary.
...
Using the monomorphic type encoding that uses the SMT array theory by default,
gives us extensionality.
To remain consistent across type encodings, extensionality is desirable.
...
It introduces additional functions and axioms for map types and map behavior.
To use the VC conclusion (the weakest precondition),
we need to satisfy the antecedent first.
This yields formal goals that our map semantics must satisfy.
...
These differ greatly with different type encodings.
...
These functions must satisfy the following axioms: \footnote{simplified for clarity}
...
These functions also play a crucial role in satisfying the VC map axioms \ref{sec:vc-map-axioms},
...
The remaining question is how to define \texttt{wf}.
...
This means that if two map values agree for each key,
then they should be represented by the same Isabelle representation.
The current definition has two issues in satisfying this property.

\paragraph{Problem 1: Low-level map as high level value}
Consider a \texttt{[int]int} map in Boogie.
...
Therefore, we enforce a property of the following form:
...
For this reason, we made the \texttt{val} datatype generic (Section~\ref{sec:datatype-construction}).
...
We now focus on \texttt{"selectImplAux (Inr (Inl (FunL m \_ \_))) (Inr (Inr k)) = Inr (m k)"}.
...
we need to show that it satisfies the goals we set,
especially from the VC map axioms \ref{sec:vc-map-axioms}.

These are the results we managed to prove, leaving out the proofs for brevity:
...
These lemmas show that our definition satisfies the required properties.
However, they involve many well-formedness assumptions.
We can eliminate them by baking these constraints into our type, as shown in Section~\ref{sec:typedef-lifting}.
...
With this type and these lemmas, we define a suitable semantics for maps,
that may have three levels of nesting in their domain,
that satisfies our goals \ref{sec:goals-to-support}, especially the VC map axioms \ref{sec:vc-map-axioms}.
