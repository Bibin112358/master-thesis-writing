...
\subsubsection{Preceding work by Parthasarathy}
...
\subsection{Type Encoding \& Extensionality}
...
is to support proof generation of Boogie files generated by Dafny.
That is why we focused on analyzing the Boogie files generated by Dafny,
...
which we explain in \ref{sec:cantor}, where we discuss why this is challenging.
Unfortunately, Dafny has a case, where it uses keys and values as maps.
The good thing is that it seems to use only one level of nesting on the keys,
so it is fine if we only support two levels of nesting of maps on the map keys.

Another limitation that we allow is that the semantics might be program dependent.
This means that there might not be a full formal semantic definition for all programs,
...
Multi-arity maps can easily be transformed to single-arity maps,
...
our map semantics need to satisfy the \emph{map axioms} of the VC generated by Boogie.
Lastly, we want our maps to be extensional. % TODO ref extensionality
...
we need to fulfill the antecedent first.
This gives us some formal goals, our map semantics need to fulfill.
...
Since the preceding work was built on polymorphic type encoding,
...
\subsubsection{Our VC antecedent change}
...
These functions will also play a crucial part in fulfilling the VC map axioms \ref{sec:vc-map-axioms},
...
the result should be constrained again, i.e. satisfy our desired properties.
...
We can see that the definition of \texttt{wf} is dependent on \texttt{wf} of other values.
...
This also makes intuitive sense, since we are only able to show that values are \texttt{wf},
but not that there are \texttt{¬wf}.
So our solution is to make the property stronger, by dropping the antecedent:
...
and a map case, where we can have our additional properties as the antecedents for implying \texttt{wf}.
...
These lemmas show that our definition fulfills the required properties.
...
on how to bake our well-formedness into a type.

We now need to "lift" our select and store functions,
...
that may have three levels of nesting in their domain,
that fulfill our goals \ref{sec:goals-to-support}, especially the VC map axioms \ref{sec:vc-map-axioms}.
