...
Another limitation we allow is that the semantics might be program dependent.
...
the number of nesting levels in maps that we extract from a given Boogie program.
...
These differ greatly across type encodings.
...
This shows that store behavior corresponds to updating at a position.
...
So we might have two maps that both capture the same behavior on integers,
but might differ on booleans.
Therefore, we also want to make sure that all maps that capture the same behavior in Boogie,
...
Values outside the intended domain are not only those with the wrong type,
but also those that are not well formed.
