...
\paragraph{Program Verification.} Program verification is a field in computer science
...
\paragraph{Translational Verifiers.} Modern program verifiers are translational, meaning they take a higher-level specification or language
...
\paragraph{Boogie.} For verification, one might take a practical programming language, such as Python,
...
% TODO picture from presentation


\subsection{Motivation}
\label{sec:motivation}

\paragraph{Formal Guarantees about Translations.} The ultimate goal of program verification is to guarantee the correctness of a program.
...
\paragraph{Prior Work.} There has been significant prior work in this space by Parthasarathy, Gaurav, et al.~\cite{parthasarathy2021formally}, 
...
\paragraph{Extending the Supported Subset.} The prior work established an immense foundation for certifying this translation.
...
\paragraph{Official Resource}

A good resource for the boogie language is the paper by K. Rustan M. Leino \cite{leino2008boogie}, describing the language,
which is also offically linked on their GitHub page
\footnote{https://github.com/boogie-org/boogie}.
Unfortunately, it seems that it is not up-to-date with the development of the langauge
...
It is still a good basis to start from, as it seems to be the most complete reference we found.

\paragraph{Boogie} This language is similar in style of Viper and Dafny,
but with less higher-level features.
It resembels a procedural language,
with assignements, control-flow statements (\texttt{if}, \texttt{while}, \texttt{goto}),
side-effect free functions and procedures, which may be polymorphic.
Being an IVL, it also has \texttt{assume}, \texttt{assert}, \texttt{axiom} statements, that way also use quantifiers.
These are for encoding the specification to define what the correct intended behaviour of the program should be.
We will not delve into too much into the details of the language.
We refere interested readers to the resources pointed before.
Here is an example to get a feeling for the language:

\begin{verbatim}
// code inspired from Dafny's encoding
...
\paragraph{Polymorphic}
We can see, that the function \texttt{lookup} has type parameters between \texttt{<>},
meaning it is a polymorphic function.

\paragraph{Type Declaration and Abstract Values} Boogie allow you to declare new types
with the \texttt{type} syntax. We will refer to values of these types as \emph{abstract values}.
The type decleration may take type parmeters, that can be used to create new types with the given instantiation,
as seen by the example of \texttt{Array}.

\paragraph{Type Synonym}
The \texttt{type} keyword can also be used to create type synonyms instead of new types.

\paragraph{Axiom and Quantifiers}
Boogie allows to introduce global assumption via the \texttt{axiom} keyword.
That means, we will assume this property holds and disregard any program execution that do not satisfy this axiom.
Ofter these axioms are used in conjuction with quantifiers like \texttt{forall}.
This lets one define a property for every value of a certain type.

\paragraph{Free}
We have decorated the procedure \texttt{positive} with the attribute \texttt{free ensures}.
The latter keyword is used to specify a post-condition of a procedure or function,
meaning this assertion should hold at the exit of the funciton.
The first keyword is telling Boogie to not require the assertion checks,
but you may still use it as an assumption when it is called.


\subsection{Boogie Maps}
\label{sec:semantics-boogie-maps}

The most relevant language constract for this project are Boogie maps,
since our contribution is to support them.
Let's start by illustrating how maps are used in Boogie.
...
With the syntax \texttt{var m:[D] R}, we can declare a map
with domain type \texttt{R} and range type \texttt{R}.
...
The domain and range types can themselves be map types,
thus enabling nested map types.
Multi-arity maps are discourage, according to this source
\footnote{https://boogie-docs.readthedocs.io/en/latest/LangRef.html\#inconsistent-assumptions}.
They can be transformed into nested single-arity maps.
...
Maps can also be polymorphic, i.e. have a type parameters:
...
We build on the prior work done by Parthasarathy et al.~\cite{parthasarathy2021formally}.

\paragraph{Overall Approach} Proving the implementation of the Boogie verifer would be a hard task.
It is a large and complex code bsae, that keeps evolving over time.
So the approach by the prior work was to instead validate individual verification runs.
Meaning the goal was to show that the validity of the outputed VC implies the correctness of the input program.

\paragraph{Formal Semantics} We also have to define what it means for a Boogie program to be correct.
For that need a formal semantics of how Boogie is to behave.
Since this did not exists before, the work has formalized part of the Boogie semantics
in a interactive theorem prover, Isabelle/HOL.

\paragraph{Proof Generation Module}
To generate this proof the work has instrumented the code base of the Boogie verifier.
This module then produces and embedding of the Boogie program and the VC in Isabelle/HOL.
It also gathers key information during the translation to be able to produce the desired proof.
See Figure~\ref{fig:proofgen-pipeline} for an illustrative diagram.
...
\begin{figure}[t]
  ...
  \end{tikzpicture}
  \caption{The Boogie verifier translate an input program to a VC.
  The proof generation module in the verifier, embeds both of them in Isabelle.
  Additionally, it generates a proof showing
  that the validity of the VC implies the correctness of the input program, defined by the foundational formal semantics.
...
% TODO cite correctly
Let's look at the overall structure of the Isabelle embedding of the generated VC,
...
So we have an implication, telling us that a modelling,
that fulfills certain assumptions, imply a weakest precondition.
This weakest precondition is an essential part to show the correcteness of our input program.
Therefore, it is necessary, that our formal semantic modelling of Boogie,
fulfills these example.
Thus this also gives us some formal goals the semantics should strive for.


\subsection{Type Encoding \& Map Extensionality}

Boogie has three different way of encoding of types in polymorphic programs,
as can be seen if we run \texttt{boogie /help}
...
The default has changed over time from predicate to monomorphic type encoding.
\footnote{https://github.com/boogie-org/boogie/pull/732}
The prior work was build, when the default was the predicate type encoding,
and works better with that type encoding.
According to the README of the prior work
\footnote{https://github.com/viperproject/boogie-proofgen/?tab=readme-ov-file\#supported-subset},
it does not support type declarations without predicate type encoding,
as they are encoded differently in the VC.

To force the predicate type encoding, one for one use the flag \texttt{/typeEncoding:p}
and have polymorphic code in the input.
If none is existence, one may add \texttt{function dummy<T>(x: T) : T;},
which does not have to be used anywhere.
Otherwise, the Boogie verifier overrides that type encoding option and monorphizes the types, as mentioned above in the help message.


The type encoding also have a huge effect on the VC encoding of maps, even if they are monomorphic.
One important difference is, that monomorphic type encoding allows maps to be modeled with SMT arrays.
Since SMT arrays are extensional, then follows than in this case Boogie maps are extensional as well.
\footnote{
This is one of the outdated facts in the "This is Boogie 2"~\cite{leino2008boogie},
wher it claims that maps are not guaranteed to be extensional.}
Extensionality means, that we consider two maps equal, if they behave the same,
i.e. if for all keys in their domain, the select operation results in the same return value.
...
It is used to check if a mathematical formual is satisfiable
under practical practical background theories, which help with modeling common things,
like arrays or real numbers.
This format is used to encode the VC output of the Boogie verifier.

There is a standardization for this language, called SMT-LIB.
One can find more information on the The SMT-LIB Standard paper~\cite{barrett2010smt}.
There are also some useful online tutorials
\footnote{https://www.philipzucker.com/z3-rise4fun/guide.html}
\footnote{https://microsoft.github.io/z3guide/docs/theories/Arrays/}
\footnote{https://theory.stanford.edu/~nikolaj/programmingz3.html}


\subsection{Interactive Theorem Provers (ITP)}

The proof generation produces its certificate in the proof language of Isabelle/HOL.
This is a Interactive Theorem Prover (ITP).
Other known ITPs include
Rocq \cite{huet1997coq} \footnote{https://rocq-prover.org/} or
Lean \cite{de2015lean} \footnote{https://lean-lang.org/}.
Such systems help with the construction of mathematical proofs on a computer,
and check that such mechanized proofs are indeed correct.

\subsubsection{Isabelle/HOL} In contrast to many other ITP,
Isabelle/HOL is not based on dependent type theory.
Isabelle itself is a meta framework to build logical systems.
Isabelle/HOL is such an implementation,
where HOL stands for higher order logic.
They do this by combining functional programming
\footnote{with syntax similar to Haskel} with logic.
We refer to the extensive tutorial \cite{nipkow2002isabelle} for more information,
as well as their website \footnote{https://isabelle.in.tum.de/}.
Here we will shortly explain a few of the less basic features we use for this project:

\paragraph{Sum Type} \footnote{https://isabelle.in.tum.de/library/HOL/HOL/Sum\_Type.html}We can combine two existing types to create a new type.
For example example \texttt{int + bool} represent the type of integers \emph{and} booleans.
Both sides then have their own datatype constructors,
\texttt{Inl} to transform the left type,
and \texttt{Inr} to transform the right type into the sum type.
Therefore, \texttt{Inl 42 :: (int + bool)} and \texttt{Inr True :: (int + bool)},
are both instances of the sum type.

\paragraph{Inductive Definitinos}
\footnote{Chapter 7 in \cite{nipkow2002isabelle}}
Inductive definitions are predicates, that describe only the postivie case.
In a sense, they allow as to mark something as true, if its buid from previous truhts.
This is especially useful, if we do not care about the negative case, and if they are hard to describe.

Let's say we want define describing if a number reaches one according to the collatz conjecture rules.
...
Such a definition is not allowed, as this function is we did not guarantee that it terminates.
Using an inductive definition, we can build the positive cases from the ground up,
simliar to a datatype:

\begin{verbatim}
inductive endsInd where
  base: "endsInd (1::nat)" |
  two: "endsInd n ==> endsInd (2*n)" |
  three: "endsInd (3*n+1) ==> endsInd n"
\end{verbatim}

As one can see, we can recurse on the inductive definition,
even with quantifiers as will see in our solution later.
Not all kind of definitions are admissible though,
for example the recursion may not appear in the negative position:
...
One reason why such a thing is not allowed, is that we cannot show the negative case
for any number, while defining the property.


\paragraph{Locale} \footnote{https://isabelle.in.tum.de/doc/locales.pdf}
We can set up locale in Isabelle/HOL.
...
Then inside this locale, we may refer to the stuff fixed by the locale.
It even allows us to do assumptions of the fixed stuff.

It we want to extend the locale to other theory files, we may do that with the \texttt{context} command:
...
And if we want to use the definitions and proofs from the locale,
with a specific values, we can instantiate them
...
In this case we instantiate \texttt{f} with \texttt{g} and \texttt{x} with \texttt{3}.
We then have to show that our instantiation fulfills the assumptions from the locale,
in this case \texttt{f x = x}.

After the interpretation, the definitions are availble, with our instantiated values.
Also for theories that import the interpreted theory.
Once interpreted, one cannot interpret again.
So, one has to make sure of importing everything one wants to to instantiate
before the interpretation, even if they are used later in other theories,
as otherwise their contexes would not be instantiated.


\paragraph{Type Definitions from Sets}
\footnote{8.5.2 in \cite{nipkow2002isabelle}}

As mentioned, Isabelle/HOL has not dependent types.
We still have a functionality to create types, based on other values,
by turning non-empty sets into types.
Actually types in Isabelle/HOL are interpreted as non-empty sets\cite{kunvcar2016types}.
Here is an example:
...
We just have to proof that the set is not empty,
as all Isabelle/HOL types are defined to be non-empty.
In fact, we this type definitino is even allowed to depent on a type parameter,
by adding a \texttt{overloaded} keyword.
...
\footnote{Might be possible with this approach\cite{kunvcar2016types}},
is have the type definition in a locale.

\paragraph{Type Class} \footnote{https://isabelle.in.tum.de/doc/classes.pdf}
Isabelle allows to add a Haskel-style type class to a type parameter of a class.
This guarantees, that certain function are available, when instantiating the type,
similar to an interface in some programming languages.
...
While a locale is more flexible, as it can talk about multiple type parameters and values too,
the crucual advantage for us is, that a type definition can use a type class in its definition.
Thus, we are able to create a type, based on other values,
like for example an inductive definition.
...
We compared the SMT output generated by Boogie v2.11.4 against that of Boogie v3.5.5.% [TODO relevant to compare proofgen?]
...
\subsection{Extensionality Requirement[TODO]}
