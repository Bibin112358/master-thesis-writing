...
Importantly, we should instantiate the locale before defining automated proof tactic methods in ML.
It turns out, that if we reference theorems in ML code inside a locale,
then they will not work correctly, when used in an instantiated context.
...
The final proof generated has some additional assumptions.
...
The first one tells us that the type of every value is closed.
This means it does not contain free type variables, like the ones used in polymorphic functions.
This makes sense and is reasonable, as type parameters appear to generalize types,
but a concrete value should have a concrete type.
...
The second one tells us that ther exsist a value for every closed type.
This made sense too, as all Boogie types are non-empty as defined in their manual. %TODO ref

The problem now is that this is not true anymore.
We can make a map type of arbritrary map levels, but our current implementation only supports $3$ levels.
That is why we have to weaken this assumption,
to not assume an impossible assumption.
We do this by first fixing a number representing the maximum map nesting level we support in our semantics locale.
Additionally, we make sure that all values actually fulfill this requirement,
whenever we want to use the locale.
...
With that we are able to weaken the non empty axiom,
by only quantifying over types with limited map levels.
We also 'strengthend' the closed type axiom, to still enable the verification of lemmas using these assumption.
Technically, we can deduce it, given our requirement from the \emph{semantics} locale.
...
With all this in place we are able to embed the input Boogie program, the generated VC
and the proposition that the validity of the VC implies the correctness of our Boogie program in Isabelle.
Many parts of the proof already verify, as they are not affected by maps,
as they only become relevant after the passification stage.
After playing with some simple examples, the proof usually failed at the stage,
where we need to \emph{glue} together a map expression expressed in Boogie
and a map expression expressed in the VC.
Most of these proofs are handled via manually implemented tactics written in \texttt{ML},
that try to solve this goal autoamtically,
using predefined simplification lemmas and proof strategies.
Since there was not enough time left, properly delving into \texttt{ML},
understanding the existing proof tactics,
and extending it with map cases were outside this scope.
Fortunately, we were able to extend it,
by supplying additional lemmas to a global lemmas variable,
solving the missing lemmas,
at least for some map test cases.
With more time, it should be feasible to support a more extensive automatic proof generation.
...
As mentioned in the previous Section~\ref{sec:proof-tactics-map},
the autoamtic proof tactic is limited at this stage.
So it does not make sense to create an extensive test case yet, to test this on.
Nonetheless, we can evaluate it on some manually created example tests
and some modified existing tests in the Boogie and Proofgen repository involving maps.


\subsubsection{Forcing Polymorphic Type Encoding}

Before we show some examples, a quick note on how to run it.
As layed out in the Section~\ref{sec:limitations},
we currently only support single-arity maps,
and do not support the select map assign operation.
So we first have to manually convert this into equivalent code, before running the tool.

% TODO move this section?
Another part is, that the proof gen module only works on the polymorphic type encoding. % TODO ref default type encoding change
For that we need to pass the command line argument \texttt{/typeEncoding:p}.
This is not enough, as Boogie automatically enforces monomorphic type encoding,
if it does not detect any polymorphic types.
So for our purposes, we have to include some polymorphic code to ensure our desired type encoding:
...
Two managed to verify, \texttt{sum\_of\_arary} and \texttt{max\_of\_arary\_v1}.
...
Boogie allows for multiple keys, see Section~\ref{sec:semantics-boogie-maps}
Our semantics, and therefore also the proof generation, is limited to just one key.
As already mentioned, we can transform such maps into single-arity map,
by having the codomain be a map, that uses the next key.
That and to make the project easier and more feasible we decided on that limitation.
Some initial ideas relied on this assumption,
as this helped ensure, that the domain only used lower levels
and that either the domain or codomain is at their maximal potential level.
Using the well formed type definition and inductive definition, see Section~\ref{sec:wf-definition},
it should be possible to support multi-arity maps natively.
We have to change the domain to a list of types,
and \texttt{FunL} would take a list of values as its domain.
The crucial change in \texttt{wf\_L} is, that \emph{all} domain types, have levels at most $n-1$,
and that \emph{at least one} has level $n-1$.
This should still satisfy our desire properties.
If this would not work, then another approach is to have a automatic transfer of multi-arity maps to single arity maps, essentially treating it as syntactic sugar.


\subsubsection{No Map Assign Statement}
Here too, the map assigment operator, see Section~\ref{sec:semantics-boogie-maps},
could be treated as syntactic sugar, being part map store and variable assignement.
Again, out of simplicity for the initial implementation we left it out for now.
The syntax is convinient though, especially for more complext map types:
...
This is mainly a sake of implementation.
The semantics would need a new command for map assignement.
This has to be extended for all relevant definitions and lemmas,
and see, if they still pass or cause any unexpected issues.
It this is no possible, we can alternatively try to transform it on the front-end side,
and compile it to simpler expressions.
Extending the semantics seems prefferable, as it closer to the input program,
and has stronger guarantees, as the front-end transformation could be buggy.


\subsubsection{No Lambda Expressions}

One thing, that make Boogie maps interesting, is the ability to assing lambda expressions to them, see \ref{sec:semantics-boogie-maps}.
The current semantics layed the ground-work, but there is still a lot to be done to support this.
Naturally, one would have to create a lambda expression datatype in the semantics first.
The harder part is, to transform this lambda expression into a function that fits \texttt{FunL},
and then transform it to a valid well formed map value.
The main challenge is to show, that the resulting transformation satisfies all the well formedness property we desire,
while ensuring, that the intented behavior is captured.
We have shown the well formedness of two lambda functions:
\begin{verbatim}
  (lambda x : int :: x + 1) : [int]int
  (lambda f : [int]int :: (lambda x : int :: f[x] + 1) ) : [[int]int][int]int
\end{verbatim}

Showin the well formedeness of these already shows the complexity of
dealing with the stratificaiton, sentinel values and type safety.
One would have to first come up how to simplify this process and the automate this generation and proving.
This sounds like a promising work for the future.


\subsubsection{Monomorphic Map Types}
Arguably, the most interesting feature of Boogie maps, are that they may use polymorphic types, see \ref{sec:semantics-boogie-maps}.
This feature even allows to store an old version of one map in the next,
all having the same type.
This leads to impredicative polymorphism and recursive types.
It is even possible to assign polymorphic lambdas.

This completely breaks our stratication approach \ref{sec:stratification-solution},
as we cannot have a map of level $n$ have a domain of level $n$ as well.
It is not even clear, in which cases such maps are logically consistent.
To support such maps one would have to use a new approach.
The preceeding work also mentions, that such weird use cases are not used in practice,
and one might limit the generality of the semantics of these maps for more practical use cases.


\subsubsection{Polymorphic Type Encoding}
The entire proof generation module is based on the polymorphic type encoding of Boogie.
At the time of the preceeding work, this has been the default type encoding,
but in the mean time the default switched to monomorphic type encoding.
Working on supporting monomorphic type encoding in proof generation might help the map semantics.
This gives us more restrictive axioms in the verification condition in terms of SMT Array Theory % TODO ref.
This might help with having a simpler version of the map semantics, and thus potentially enabling more general map behaviors.


\subsubsection{Type Helper for Maps}
Some parts of the foundational boogie semantic, deal with type inference and type safety.
These would also need to be extended to include map cases.
After having done that, one could implement the map cases for the proof tactics regarding types in \texttt{ML}.
This would enable more generated proofs to verify.


\subsubsection{Current Automatic Proof Tactic Strength}
Generally, there is just rudimentary support for maps in the proof tactics.
We included some integer and boolean specific lemmas to verify common use cases.
Understanding the tactics and extending it in a more general and exhaustive way
could lead to more generated proofs to verify.
