...
The project was about evolving and extending certificate generatation for a program verifier.
We build on the prior work of Parthasarathy et al.~\cite{parthasarathy2021formally},
which build semantic formalization and certificate generation for a core subset of the Boogie language,
in the proof language of Isabelle/HOL.
We have evolved it by updating the underlying Boogie code base to the newest version,
fixing related issues
and thusenabling the tool to be run and devloped on for future contributions.
For the extension part, we have analyzed which previously identified features are most relevant for supporting next,
resulting in supporting maps.

From there we have designed formal semantics for monomorphic, single-arity Boogie maps
with finite domain nestings by shallow embedding them to Isabelle functions,
with custom contrains.
Showing the possibilities and limitations of shallow embedding functions,
in contrast to a deeper embedding with expressions and variable binders.
Our map semantics satisfy extensionality and minimally changed predicate verificatiion condition axioms.
We extended the proof generation with some rudementary support for maps,
enabling the first sucessfull verifiable certificate generation,
showing the validity of our approach.
Extending proof tactics to support more auto verification of certificate,
extending the semantics to model more general maps behvaviors,
such as lambda expressions, arbitrary domain nestings and polymorphic maps remain open challanges for future work.
