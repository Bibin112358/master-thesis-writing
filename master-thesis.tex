...
with domain type \texttt{K} and the codomain type \texttt{V}.
...
Thus, a map behaves like a function that are first-class values and may be updated.
...
This is a powerful feature, and its support outside the scope of this thesis,
...
In Section~\ref{sec:cantor} we discuss why this is challenging.
...
so supporting two levels of map domain nesting is sufficient for now.
...
we need to support \emph{two levels of nesting} on the domain types for maps.
...
\footnote{we simplfified the axioms a bit for clarity}
\begin{itemize} 
...
These guarantee that $\MapTypeInvK$ and $\MapTypeInvV$ are actually
inverse functions of the map type constructor $\MapType$.
...
If we select from a map with the correct domain type,
then the result is of the corresponding codomain type.
...
This shows the behaviour of store is updating at a position.
...
This shows that the store does not affect the other positions.


\subsubsection{Our VC antecedent change}
\label{sec:our-vc-change}

The current axiom, talking about the map update \eqref{ax:map-update-strong},
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Level $n$: Maps where the domain has of levels $\le n-1$ and codomain has levels $\le n$.
\end{itemize}
...
\texttt{[int][[int]int]int} (a map of level 2, with domain of level 0 and codomain of level 2) or \texttt{[[[int]int]int]int} (a map of level 3, with domain of level 2 and codomain of level 0).
...
That means depending on the input program, we could get a different semantics.
...
especially regarding $\MapType, \MapTypeInvK, \MapTypeInvV$.
...
We will need to restrict them, with some additional well-formedness properties, \texttt{wf},
...
After removing \texttt{wf}, the stronger version is a derived property in combination with the other properties.
...
The other two map cases are analogous.
