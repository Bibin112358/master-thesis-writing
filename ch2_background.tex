...
\footnote{\url{https://github.com/boogie-org/boogie}}.
Unfortunately, the paper is no longer entirely up-to-date with the modern development of the language.
\footnote{\url{https://boogie-docs.readthedocs.io/en/latest/LangRef.html}}\textsuperscript{,}
\footnote{\url{https://github.com/dafny-lang/dafny/issues/4005\#issuecomment-1570363166}}
...
As an IVL, it also includes \lstinline[columns=fixed]{assume}, \lstinline[columns=fixed]{assert},
...
However, multi-arity maps are generally discouraged according to this documentation~\footnote{\url{https://boogie-docs.readthedocs.io/en/latest/LangRef.html\#map-types}},
...
Maps can also be polymorphic, meaning they can accept type parameters:\footnote{Examples taken and adapted from \url{https://ethz.ch/content/dam/ethz/special-interest/infk/chair-program-method/pm/documents/Education/Courses/SS2017/Program\%20Verification/08-Boogie.pdf}.}
...
The goal is to formally prove that the validity of the outputted VC
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node at (9.5,-3.55) {\includegraphics[width=1.15cm]{uploads/isabelle_transparent.png}};
  \end{tikzpicture}
...
and SMT solvers are used to determine if such a formula is valid.
...
and various online resources provide comprehensive tutorials on writing and evaluating SMT queries.~\footnote{\url{https://www.philipzucker.com/z3-rise4fun/guide.html}}~\textsuperscript{,}~\footnote{\url{https://microsoft.github.io/z3guide/docs/theories/Arrays/}}\textsuperscript{,}~\footnote{\url{https://theory.stanford.edu/\~nikolaj/programmingz3.html}}


\section{Type Encoding \& Map Extensionality}
\label{sec:type-encoding}

When an input program utilizes polymorphism, such as polymorphic functions, procedures, or maps,
the Boogie verifier must decide how to represent these types within the VC.
Because SMT solvers usually operate on fixed, monomorphic sorts, the verifier must apply a strategy to "encode" these polymorphic types.

There are two primary approaches to this encoding:
\begin{itemize}
\item \textbf{Predicate Type Encoding:} The verifier creates a single universal \emph{sort} in the VC to represent all values and another for all types. Each type from the input program is represented as a constant, and a global \texttt{type} function is used to map values to their respective types. This keeps the logic polymorphic but relies on axioms to govern type safety.
\item \textbf{Monomorphic Type Encoding:} The verifier creates a distinct \emph{sort} in the VC for every concrete type used in the program. This process, often called monomorphization, effectively "flattens" the polymorphism by generating a specific version of the code for each type instantiation.
\end{itemize}

There is also a third way, arguments type encoding, but which is not relevant for us.
We can configure these encodings using the Boogie CLI, as detailed by the \texttt{boogie /help} command:
...
\footnote{\url{https://github.com/boogie-org/boogie/pull/732}}
...
\footnote{\url{https://github.com/viperproject/boogie-proofgen/?tab=readme-ov-file\#supported-subset}},
...
Because SMT arrays are inherently extensional, it follows that under this encoding, Boogie maps are extensional as well.\footnote{This contradicts an outdated assertion in the paper "This is Boogie 2" \cite{leino2008boogie}, which claims maps are not guaranteed to be extensional.}
...
Other prominent ITPs include Rocq\footnote{\url{https://rocq-prover.org/}}
\cite{huet1997coq} (formerly known as Coq)
and Lean\footnote{\url{https://lean-lang.org/}} \cite{de2015lean}.
...
and the project website\footnote{\url{https://isabelle.in.tum.de/}} for deeper information.
Here, we briefly introduce a few advanced features used heavily throughout this project:

\paragraph{Sum Types}\footnote{\url{https://isabelle.in.tum.de/library/HOL/HOL/Sum\_Type.html}}
...
\paragraph{Locales}\footnote{\url{https://isabelle.in.tum.de/doc/locales.pdf}}
...
\paragraph{Type Classes}\footnote{\url{https://isabelle.in.tum.de/doc/classes.pdf}}
